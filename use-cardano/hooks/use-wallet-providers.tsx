...
const useWalletProviders = (options: UseCardanoOptionsWithDefaults) => {
  ...
  useEffect(() => {
    ...
    interval.current = setInterval(() => {
      ...
      const providers : any = Object.keys(window.cardano)
        .map((key) => ({
          key,
          ...window.cardano[key],
        }))
      ...
      if (providers.some((p:any) => p.key === providerToConnectTo)) {
        const connectedProvider = providers.find((p:any) => p.key === providerToConnectTo)
